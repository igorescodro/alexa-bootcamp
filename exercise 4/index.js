...
const BuyProductIntent = {
    canHandle(handlerInput){
        const request = handlerInput.requestEnvelope.request;
        return request.type === 'IntentRequest' && request.intent.name === 'BuyProductIntent';
    },
      handle(handlerInput) {
        const slots = handlerInput.requestEnvelope.request.intent.slots;
        const product = slots.device.value;    //slots['product'].value;
        const color = slots.color.value;        //slots['color'].value
        const country = slots.country.value;    //slots['country'].value;
        
        const speechText = `Great! Your ${product} in ${color} will be delivered to ${country} in 10 business days`;
